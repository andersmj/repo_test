 ...
 *
 * SVN $Revision: 6533 $
 * SVN $Date: 2014-06-06 16:08:00 +0100 (Fri, 06 Jun 2014) $
 */
/*=========================================================================*//**
  ...
  this driver with the exception of the SmartFusion IOMUX configuration.
  SmartFusion allows multiple non-concurrent uses of some external pins through
  ...
  FPGA fabric. The MSS I2Cs serial signals are routed through IOMUXs to the
  SmartFusion device external pins.
  
  The IOMUXs are automatically configured correctly by the MSS configurator tool
  in the hardware flow when the MSS I2Cs are enabled in that tool.
  You must ensure that the MSS I2C peripherals are enabled and configured in the
  SmartFusion MSS configurator if you wish to use them. For more information on
  the MSS I2C IOMUXs, refer to the "Inter-Integrated Circuit (I2C) Peripherals"
  section of the SmartFusion Microcontroller Subsystem (MSS) User’s Guide.
  
  The base address, register addresses and interrupt number assignment for the
  MSS I2C peripherals are defined as constants in the SmartFusion CMSIS PAL.
  You must ensure that the latest SmartFusion CMSIS PAL is included in the
...
/*-------------------------------------------------------------------------*//**
  ...
  @code
    ...
    {
        MSS_I2C_init( &g_mss_i2c0, SLAVE_SER_ADDR_0, MSS_I2C_PCLK_DIV_256 );
        MSS_I2C_init( &g_mss_i2c1, SLAVE_SER_ADDR_1, MSS_I2C_PCLK_DIV_256 );
    }
  @endcode
 */
void MSS_I2C_init
...
/*-------------------------------------------------------------------------*//**
  ...
  Example:
    ...
    would be called in a Cortex-M3 based system. MSS_I2C_system_tick() is called
    for each MSS I2C peripheral from the Cortex-M3 SysTick timer interrupt
  ...
  @code
    ...
    {
        MSS_I2C_system_tick(&g_mss_i2c0, SYSTICK_INTERVAL_MS);
        MSS_I2C_system_tick(&g_mss_i2c1, SYSTICK_INTERVAL_MS);
    }
  @endcode
 */
...
/*-------------------------------------------------------------------------*//**
  The MSS_I2C_set_gca() function is used to set the general call acknowledgement
...
/*-------------------------------------------------------------------------*//**
  ...
  acknowledgement bit of an MSS I2C slave device. This will stop the I2C slave
...
/*-------------------------------------------------------------------------*//**
  ...
  override the weak stub function implemented in the CMSIS-PAL startup code:
  ...
  @code
    #define SLAVE_SER_ADDR     0x10u
    __attribute__((__interrupt__)) void I2C0_SMBAlert_IRQHandler( void )
    {
        // MSS I2C 0 application specific SMBALERT code goes here ...
    }

    __attribute__((__interrupt__)) void I2C0_SMBus_IRQHandler( void )
    {
    ...
    void main( void )
    {
        MSS_I2C_init( &g_mss_i2c0t, SLAVE_SER_ADDR, MSS_I2C_PCLK_DIV_256 );
...
/*-------------------------------------------------------------------------*//**
  ...
  @param irq_type
    The irq_type parameter specifies the SMBUS interrupt that will be disabled.
...
/*-------------------------------------------------------------------------*//**
  ...
  @code
    ...
    void main( void )
    {
        ...
        MSS_I2C_suspend_smbus_slave( &g_mss_i2c0 );

        ...

        // Re-enable SMBus slaves
        MSS_I2C_resume_smbus_slave( &g_mss_i2c0 );
    }
  @endcode
 */
...
/*-------------------------------------------------------------------------*//**
  ...
  @code
    ...
    void main( void )
    {
        ...
        MSS_I2C_set_smbus_alert( &g_mss_i2c0 );

        ...

        // Once we are happy, drop the alert
        MSS_I2C_clear_smbus_alert( &g_mss_i2c0 );
    }
  @endcode
 */
...
/*-------------------------------------------------------------------------*//**
  ...
  @code
    ...
    void main( void )
    {
        ...
        MSS_I2C_set_user_data( &g_mss_i2c0, (void *)&channel_xdata );

        ...

        // Retrieve location of user data and do some work on it
        p_xdata = (app_data_t *)MSS_I2C_get_user_data( &g_mss_i2c0 );
        if( NULL != p_xdata )
        {
            p_xdata->foo = 123;
        }
