/***************************************************************************//**
 * (c) Copyright 2008-2013 Microsemi Corporation.  All rights reserved.
 * 
 ...
 *
 * SVN $Revision: 6231 $
 * SVN $Date: 2014-03-15 07:57:19 -0700 (Sat, 15 Mar 2014) $
 */
/*=========================================================================*//**
  ...
  Configuration
    An MSS SPI block can operate either as a master or slave SPI device. There
    ...
    remote SPI slave device which the MSS SPI block will communicate with. It 
    is used to provide the following information about each SPI slave’s 
    ...
    clock edge will be used to sample the data signal (Motorola SPI modes), or
    it defines whether the SPI block will operate in Texas Instruments (TI)
    synchronous serial mode or in National Semiconductor (NSC) MICROWIRE mode.
    ...
    signal (Motorola SPI modes), or it defines if the SPI block will operate in
    TI synchronous serial mode 
      Note: The MSS SPI cannot be configured to operate as a NSC MICROWIRE
            protocol mode slave.
  ...
  SPI master block transfer control
    ...
    MSS_SPI_set_slave_select(). This causes the relevant slave select line to
    become asserted while data is clocked out onto the SPI data line.
    A call is then made to function MSS_SPI_transfer_block(). The parameters
    of this function specify:
    ...
    the transfer is purely a block read transfer. Alternatively, the number of
    bytes to be received can be set to zero to specify that the transfer is
    purely a block write transfer.
    The function MSS_SPI_clear_slave_select() can be used after the transfer is
    complete to prevent this slave select line from being asserted during
    subsequent SPI transactions. A call to this function is only required if the
    master is communicating with multiple slave devices.
    Note:   When performing block transfers using the A2F500 and A2F060
            SmartFusion devices, the MSS SPI asserts the target slave device’s
            chip select throughout the transfer in Motorola SPI modes 1 and 3
            (i.e. protocol mode configuration is MSS_SPI_MODE1 or MSS_SPI_MODE3),
            whereas the slave select is de-asserted between each byte transfer
            in Motorola SPI modes 0 and 2 (i.e. protocol mode configuration is
            MSS_SPI_MODE0 or MSS_SPI_MODE2). A GPIO may be used to control the
            target slave device’s chip select in Motorola SPI modes 0 and 2 if
            de-assertion of the slave select between each byte transfer is not
            desired.
    Note:   When performing block transfers using the A2F200 SmartFusion device,
            it is recommended that you use a GPIO to control the target slave
            device’s chip select input. This ensures reliable operations by
            preventing the chip select signal from becoming de-asserted, before
            the transfer completes, as a result of the SPI transmit FIFO
            becoming empty. This issue may occur where a high priority interrupt
            is serviced in some other part of the system while the SPI transfer
            is taking place. This potential problem only exists for block
            transfers on the A2F200 SmartFusion device.
  ...
  SPI slave block transfer control
    ...
    the parameters to MSS_SPI_set_cmd_handler(). The command handler
    function is implemented as part of the application making use of the SPI
    driver and would typically call the MSS_SPI_set_cmd_response() function.
    The MSS_SPI_set_cmd_response() function specifies the data that will be
    returned to the master. Typically the MSS_SPI_set_slave_block_buffers()
    will have been called as part of the system initialization to specify the
    data sent to the master while the command bytes are being received. The
    transmit buffer specified though the call to MSS_SPI_set_slave_block_buffers()
    would also typically include one or more bytes allowing for the turnaround
    time required for the command handler function to execute and call
    MSS_SPI_set_cmd_response().
    Note:   The slave block transfer mode of operation cannot be used on the
            A2F200 SmartFusion device due to slave mode limitations in the SPI
            hardware block of this device.
  ...
  DMA block transfer control
    ...
    The number of bytes to be transferred is then set through a call to
...
/***************************************************************************//**
  ...
  Declaring and Implementing Slave Block Receive Handler Functions
    ...
    will indicate the number of bytes in the received block.
...
/***************************************************************************//**
  ...
  bits which select the different modes of operation for the MSS SPI. It is
  used as a parameter to the MSS_SPI_configure_master_mode() and
...
/***************************************************************************//**
  ...
  SS_SPI_configure_slave_mode() when performing block transfers between the
...
/***************************************************************************//**
  ...
  selected SPI protocol mode, frame size and clock speed.
  Note: The MSS SPI cannot be configured to operate as a NSC MICROWIRE protocol
        mode slave.
...
/***************************************************************************//**
  ...
  must be called once for each target SPI slave which the MSS SPI master wishes to
...
/***************************************************************************//**
  ...
  Note: This function cannot be used on the A2F200 SmartFusion device due to
        slave mode limitations in the SPI hardware block of this device.
  ...
  the receive handler is called, which determines the response data to be
  returned to the master during the next block transfer. The second part of the
  transaction, where the slave returns data based on the command value, is sent
  using a DMA transfer initiated from within the receive handler.
